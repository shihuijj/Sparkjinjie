...
# ./bin/hive
```
如果出现cannot access ‘/usr/local/spark/lib/spark-assembly-*.jar的错误，vi /usr/local/hive/bin/hive，将sparkAssemblyPath改为:sparkAssemblyPath=ls ${SPARK_HOME}/jars/*.jar 。如果显示配置文件缺漏，建议重新解压安装hive。启动成功以后，就进入了“hive>”命令提示符状态，可以输入类似SQL语句的HiveQL语句。下面，我们要在Hive中创建一个数据库dbtaobao，命令如下：
```
